...
function HealBot_Action_UpdateAggro(unit,status,threatStatus,hbGUID,threatPct)
    ...
    if not barName or not uName then 
        return 
    end    
    ...
    elseif Healbot_Config_Skins.HighLightActiveBar[Healbot_Config_Skins.Current_Skin]==1 and status=="highlight" and not HealBot_Aggro[hbGUID] then 
        ...
        if HealBot_Hightlight and HealBot_Hightlight==hbGUID and HealBot_UnitThreat[hbGUID]<3 then 
            HealBot_UnitThreat[hbGUID]=-1
        end
    end
    if status and 
       (HealBot_UnitThreat[hbGUID]>(Healbot_Config_Skins.AggroAlertLevel[Healbot_Config_Skins.Current_Skin]-2) or 
        Healbot_Config_Skins.ShowAggroBarsPct[Healbot_Config_Skins.Current_Skin]==1 or
        HealBot_Aggro[hbGUID]=="h") and
       (Healbot_Config_Skins.ShowAggroBars[Healbot_Config_Skins.Current_Skin]==1 or Healbot_Config_Skins.HighLightActiveBar[Healbot_Config_Skins.Current_Skin]==1) then
...
function HealBot_Action_EndAggro()
    for hbGUID,_ in pairs(HealBot_Aggro) do
        if HealBot_Action_retAggro(hbGUID) and HealBot_UnitID[hbGUID] then
...
function HealBot_Action_RetUnitThreat(hbGUID)
    return HealBot_UnitThreat[hbGUID] or -2
end
...
function HealBot_Action_ShouldHealSome(hbGUID)
    if HealBot_Enabled[hbGUID] then 
        ...
        for xUnit,_ in pairs(HealBot_Unit_Button) do
            if HealBot_Enabled[HealBot_UnithbGUID[xUnit]] then
                return true
            end
        end
    end
...
--    if not uName then return end
    ...
    ebuicon15 = getglobal(ebubar:GetName().."Icon15");
...
function HealBot_Action_HBText(hlth,maxhlth,unitName,unit,healin, hbGUID)
    ...
    if HealBot_Action_UnitIsOffline(hbGUID) and uName then
...
--  if type(button)~="table" then DEFAULT_CHAT_FRAME:AddMessage("***** "..type(button)) end
    ...
    if UnitExists("target") and HealBot_UnithbGUID["target"]==hbGUID and button.unit~="target" and HealBot_Unit_Button["target"] then
...
local abtSize = {[0]=1,[1]=1,[2]=1,[3]=2,[4]=2,[5]=2,[6]=3,[7]=3,[8]=3,[9]=3,[10]=4,[11]=4,[12]=4,[13]=4,[14]=4,[15]=5}
    ...
    if barType=="bar" then
        if Healbot_Config_Skins.ShowAggro[Healbot_Config_Skins.Current_Skin]==0 and Healbot_Config_Skins.HighLightActiveBar[Healbot_Config_Skins.Current_Skin]==0 then abSize=0 end
    ...
    --bar.txt:SetTextHeight(btextheight);
        ...
        b:SetWidth(bwidth)
        for x=2,14 do
            icon=getglobal(bar:GetName().."Icon"..x);
            icont=getglobal(bar:GetName().."Count"..x);
            iconta=getglobal(bar:GetName().."Count"..x.."a");
        ...
        icon15 = getglobal(bar:GetName().."Icon15");
        icon16 = getglobal(bar:GetName().."Icon16");
        icon15t = getglobal(bar:GetName().."Count15"); 
        icon15ta = getglobal(bar:GetName().."Count15a");   
    ...
    elseif barType=="hbfocus" then
        ...
        HealBot_Action_hbFocusButtonBar.txt = getglobal(HealBot_Action_hbFocusButtonBar:GetName().."_text");
...
function HealBot_Action_CheckRange(unit, button)
    ...
    if HealBot_UnitStatus[unit]>0 then
        ...
        if HealBot_UnitRange[unit]==-2 then
            ...
            HealBot_Action_RefreshButton(button, HealBot_UnithbGUID[unit])
...
function HealBot_Action_SetHealButton(index,unit,hbGUID)
    if UnitClass(unit) and hbGUID then
        ...
        if HealBot_Unit_Button[unit]~=shb or not shb.guid then
            ...
            if not HealBot_ClassColR[hbGUID] then 
                HealBot_Action_ClassColour(hbGUID, unit) 
                HealBot_Enabled[hbGUID]=false
            end
...
function HealBot_Action_SetTargetHealButton(unit,hbGUID)
    if UnitClass(unit) and hbGUID then
        shb=getglobal("HealBot_Action_HealUnit56");
        if not HealBot_UnitSpec[hbGUID] then
            HealBot_Unit_Button[unit]="init"
            HealBot_UnitSpec[hbGUID] = " "
            HealBot_UnitTime[hbGUID]=GetTime()
...
function HealBot_Action_SetButtonAttrib(button,bbutton,bkey,status,j)
    ...
    if sName then
        ...
        elseif strlower(sName)==strlower(HEALBOT_HBMENU) and HealBot_UnithbGUID[button.unit] then
            curGUID=HealBot_UnithbGUID[button.unit]
            button:SetAttribute(HB_prefix.."helpbutton"..j, nil);
...
function HealBot_Action_AlterSpell2Macro(spellName, spellTar, spellTrin1, spellTrin2, unit, combo)
    if not smName[combo..unit] then
        smName[combo..unit]=""
        if HealBot_Config.MacroSuppressSound==1 then smName[combo..unit]=smName[combo..unit].."/console Sound_EnableSFX 0;\n" end
        ...
        if HealBot_Config.MacroSuppressSound==1 then smName[combo..unit]=smName[combo..unit].."/console Sound_EnableSFX 1;\n" end
...
function HealBot_Action_hbmenuFrame_DropDown_Initialize(self,level)
    ...
    if level==1 then
        ...
        if HealBot_Action_RetMyTarget(HealBot_UnithbGUID[self.unit]) then
            info.text = HEALBOT_SKIN_DISTEXT;
        else
            info.text = HEALBOT_SKIN_ENTEXT;
        end
        info.func = function() HealBot_Action_Toggle_Enabled(HealBot_UnithbGUID[self.unit]); end
        UIDropDownMenu_AddButton(info, 1);

        info = UIDropDownMenu_CreateInfo();
        info.hasArrow = false; 
        info.notCheckable = true;
        if HealBot_Panel_RetMyHealTarget(HealBot_UnithbGUID[self.unit]) then
            info.text = HEALBOT_WORDS_REMOVEFROM.." "..HEALBOT_OPTIONS_TARGETHEALS;
        else
            info.text = HEALBOT_WORDS_ADDTO.." "..HEALBOT_OPTIONS_TARGETHEALS
        end
        info.func = function() HealBot_Panel_ToggelHealTarget(HealBot_UnithbGUID[self.unit]); end;
        UIDropDownMenu_AddButton(info, 1);
        
        info = UIDropDownMenu_CreateInfo();
        info.hasArrow = false; 
        info.notCheckable = true;
        if HealBot_Panel_RetPrivateTanks(HealBot_UnithbGUID[self.unit]) then
        ...
        info.func = function() HealBot_Panel_ToggelPrivateTanks(HealBot_UnithbGUID[self.unit]); end;
        UIDropDownMenu_AddButton(info, 1);
        ...
        info.func = function() HealBot_Reset_Unit(HealBot_UnithbGUID[self.unit]); end
        UIDropDownMenu_AddButton(info, 1);
        ...
        info.func = function() HealBot_Panel_AddBlackList(HealBot_UnithbGUID[self.unit]); end
        UIDropDownMenu_AddButton(info, 1);
...
function HealBot_Action_PartyChanged(HealBot_PreCombat)
    ...
    if HealBot_IsFighting and not InCombatLockdown() then 
    ...
    if not HealBot_IsFighting and HealBot_PlayerGUID then
        ...
        if not hbInitButtons then
            ...
            b=getglobal("HealBot_Action_HealUnit56")
            b.id=56
            hbInitButtons=true
        end
...
function HealBot_Action_ClassColour(hbGUID, unit)
    if HealBot_ClassColR[hbGUID] then
        return HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID]
    elseif unit and hbGUID and HealBot_UnithbGUID[unit]~=hbGUID then
        return 0.78,0.61,0.43
    elseif not hbGUID then
        return 0.78,0.61,0.43
    end
    class=nil
    if HealBot_UnitID[hbGUID] then
        _,class=UnitClass(HealBot_UnitID[hbGUID])
    elseif unit and UnitClass(unit) then
        _,class=UnitClass(unit)
    end
    if class ~= nil then
        if strsub(class,1,4)=="DRUI" then
            HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID] = 1.0,0.49,0.04
        elseif strsub(class,1,4)=="HUNT" then
            HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID] = 0.67,0.83,0.45
        elseif strsub(class,1,4)=="MAGE" then
            HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID] = 0.41,0.8,0.94
        elseif strsub(class,1,4)=="PALA" then
            HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID] = 0.96,0.55,0.73
        elseif strsub(class,1,4)=="PRIE" then
            HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID] = 1.0,1.0,1.0
        elseif strsub(class,1,4)=="ROGU" then
            HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID] = 1.0,0.96,0.41
        elseif strsub(class,1,4)=="SHAM" then
            HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID] = 0.14,0.35,1.0
        elseif strsub(class,1,4)=="WARL" then
            HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID] = 0.58,0.51,0.79
        elseif strsub(class,1,4)=="DEAT" then
            HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID] = 0.78,0.04,0.04
        else
            HealBot_ClassColR[hbGUID],HealBot_ClassColG[hbGUID],HealBot_ClassColB[hbGUID] = 0.78,0.61,0.43
        end
    else
        return 0.78,0.61,0.43
    end
...
local HealBot_Hightlight=nil
function HealBot_Action_HealUnit_OnEnter(self)
    if not self.unit then return; end
    xGUID=HealBot_UnithbGUID[self.unit]
    ...
    if Healbot_Config_Skins.HighLightActiveBar[Healbot_Config_Skins.Current_Skin]==1 and not HealBot_PlayerDead and -- and not HealBot_Aggro[xGUID] 
       (Healbot_Config_Skins.HighLightActiveBarInCombat[Healbot_Config_Skins.Current_Skin]==1 or not HealBot_IsFighting) then
        HealBot_Action_UpdateAggro(self.unit,"highlight",HealBot_UnitThreat[xGUID] or 0,xGUID)
        HealBot_Hightlight=xGUID
    end
end

function HealBot_Action_HealUnit_OnLeave(self)
    HealBot_Action_HideTooltip(self);
    if HealBot_Hightlight then
        if HealBot_Aggro[HealBot_Hightlight] and HealBot_Aggro[HealBot_Hightlight]=="h" then
            HealBot_Action_UpdateAggro(self.unit,false,nil,HealBot_Hightlight)
        end
        HealBot_Hightlight=nil
    end
    HealBot_curGUID=nil
end
...
function HealBot_Action_HealUnit_Wheel(delta)
    ...
    xUnit=HealBot_UnitID[HealBot_curGUID]
...
function HealBot_Action_hbFocusButton_OnLoad(self)
    ...
    HealBot_Action_SethbFocusButtonAttrib(getglobal("HealBot_Action_hbFocusButton"))
    b=getglobal("HealBot_Action_hbFocusButton")
    b.id=999
...
function HealBot_Action_Set_Timers(override)
    ...
    if HealBot_Config.DisableHealBot==0 and ((Healbot_Config_Skins.ShowAggroBars[Healbot_Config_Skins.Current_Skin]==1 and Healbot_Config_Skins.ShowAggro[Healbot_Config_Skins.Current_Skin]==1) or Healbot_Config_Skins.HighLightActiveBar[Healbot_Config_Skins.Current_Skin]==1) then
        if not override then
...
local HealBot_AggroBarColr = {[-1]=1, [0]=1, [1]=1, [2]=1, [3]=1, [5]=1, [6]=1, [7]=1, [8]=1, [9]=1}
local HealBot_AggroBarColg = {[-1]=1, [0]=1, [1]=1, [2]=0.1, [3]=0.1, [5]=1, [6]=1, [7]=1, [8]=1, [9]=1}
local HealBot_AggroBarColb = {[-1]=1, [0]=0.4, [1]=0.2, [2]=0.1, [3]=0.1, [5]=1, [6]=1, [7]=1, [8]=1, [9]=1}
...
function HealBot_Action_OnUpdate(self,arg1)
    HealBot_Action_Timer1 = HealBot_Action_Timer1+arg1
    HealBot_Action_Timer2 = HealBot_Action_Timer2+arg1
...
function HealBot_Action_OnDragStart(self,button)
...
function HealBot_Action_RetHealBot_ClassCol(hbGUID)
    return HealBot_Action_ClassColour(hbGUID)
end

function HealBot_Action_immediateClearLocalArr(hbGUID)
    HealBot_Enabled[hbGUID]=nil
    HealBot_ClassColR[hbGUID]=nil
    HealBot_ClassColG[hbGUID]=nil
    HealBot_ClassColB[hbGUID]=nil
    HealBot_UnitOffline[hbGUID]=nil
    HealBot_AggroBar4[hbGUID]=nil
